...
## 💎 개선 사항 정리
### Slack 알림 개선
- 기존 추가 구성에서는 알릴 데이터 포인트를 1/1 를 선택해서 넘겼는데 의문이 생겼음
- Slack에서 CPU 70% 이상일 시 경보 알람은 오지만 장애 해소 시 정상으로 변경되었다는 알림이 오지 않았음
- 이걸 해결하기 위해 다시 처음부터 경보를 수정하였는데 추가 구성에 경보를 알릴 데이터를 늘리면 됐음
- 추가 구성에서 경보를 알릴 데이터 포인트 3/3 선택한 후 `정상 → 경보` , `경보 → 정상` 으로 정상 작동
## 📊 결과 확인
